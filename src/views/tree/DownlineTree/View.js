...
export default function CustomizedTables() {
  ...
  const AgentTree = useCallback(async (Count) => {
    ...
    try {
      await api
        .get(`${Api.request.URL}/api/v2/player/downline?refcode=${id}&start=${Count === undefined ? 0 : Count}`)
        .then((res) => {
              ...
              getTree.map((row, i) => {
                ...
                return (
                  <StyledTableRow>
                    ...
                    <StyledTableCell align="left">
                      {row.groups === "AGENT" ? "DIRECT_PLAYER" : row.groups}
                    </StyledTableCell>
